...
while True:
    ...
    if result.multi_hand_landmarks:
        for i, handLms in enumerate(result.multi_hand_landmarks):
            ...
            if lmList:
                # El yönünü belirle (Right ya da Left)
                handLabel = result.multi_handedness[i].classification[0].label  # 'Right' veya 'Left'

                # Başparmak kontrolü (el yönüne göre x ekseni)
                if handLabel == "Right":
