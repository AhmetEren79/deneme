...
# %% visualize
plt.imshow(x_train[5, :].reshape(64, 32), cmap='gray')  # 45002 ve 1001
plt.show()
# %%
num_classes = 2
# Hyper parameters
...
for epoch in range(num_epochs):
    ...
    for images, labels in pbar:
        images = images.view(images.size(0), 1, 64, 32).float() / 255.0
        if use_gpu:
        ...
        # bar üstüne kayıp bilgisini yazdır
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            total += labels.size(0)
            correct += (predicted == labels).sum().item()
    print("Accuracy test %d.2f %%" % (100 * correct / total))
