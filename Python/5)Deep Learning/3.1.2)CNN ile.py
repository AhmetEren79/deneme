...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)  # dinamik flatten
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        return x
...
trainloader = torch.utils.data.DataLoader(train, batch_size=batch_size, shuffle=True, drop_last=False)

test = torch.utils.data.TensorDataset(x_test, y_test)
testloader = torch.utils.data.DataLoader(test, batch_size=batch_size, shuffle=False, drop_last=False)
...
for epoch in tqdm(range(num_epochs), desc="Training Epochs"):
    ...
    for i, data in enumerate(trainloader, 0):
        inputs, labels = data
        # reshape dinamik: son batch küçük olabilir
        inputs = inputs.view(inputs.size(0), 1, 64, 32).float()
    ...
    with torch.no_grad():
        for data in testloader:
            images, labels = data
            images = images.view(images.size(0), 1, 64, 32).float()
            if use_gpu and torch.cuda.is_available():
    ...
    tqdm.write(
        ...
        f"Loss: {avg_loss:.4f} | Train Acc: {acc_train:.2f}% | Test Acc: {acc_test:.2f}%"
    )
